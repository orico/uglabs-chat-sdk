...
from ..core.voice import generate_test_audio, play_audio_response_async, record_voice_input_async
...
class ChatSession:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            mode_instructions = (
                ...
                if self.voice_mode
                else "Type your message or use commands"
            )

            self.console.print(
                f"[bold {mode_color}]Chat ready in {mode_indicator} mode![/bold {mode_color}]"
            )
            ...
            if self.voice_mode and not is_interactive:
                self.console.print(
                    "[dim]Note: Voice recording requires interactive terminal with microphone access[/dim]"
                )
            ...
            self.console.print()
            self.console.print(
                "[dim]💡 Use ↑/↓ arrows for persistent message history, Tab for auto-completion[/dim]"
            )
            self.console.print()
    ...
    async def create_new_player(self, external_id: str) -> str:
        ...
        try:
            ...
            federated_id: str = player_data["federated_id"]
    ...
    async def send_message(self, text: str) -> None:
        ...
        try:
            ...
            async for response in self.client.send_text_interaction(
                text, audio_output=self.audio_output
            ):
                response_count += 1
                if self.debug_mode:
                    self.console.print(
                        f"[dim]Debug: Text response {response_count}: {response}[/dim]"
                    )
            ...
            if audio_responses and self.audio_output:
                if self.debug_mode:
                    ...
                    self.console.print(
                        f"[dim]Debug: Collected {len(audio_responses)} audio chunks ({total_bytes} bytes total), processing and merging before playback[/dim]"
                    )
                ...
                for i, audio_data in enumerate(audio_responses, 1):
                    ...
                    if self.debug_mode:
                        self.console.print(
                            f"[dim]Debug: Processing audio chunk {i}/{len(audio_responses)}: {len(audio_bytes)} bytes, starts with: {audio_bytes[:10].hex() if len(audio_bytes) >= 10 else audio_bytes.hex()}[/dim]"
                        )
                    ...
                    if len(audio_bytes) >= 3 and audio_bytes[:3] == b"ID3":
                        ...
                        if self.debug_mode:
                            self.console.print(
                                f"[dim]Debug: Chunk {i} has ID3 header - MP3 file[/dim]"
                            )
                        ...
                        if self.debug_mode:
                            self.console.print(
                                f"[dim]Debug: Chunk {i} starts with MP3 frame sync - treating as MP3[/dim]"
                            )
                    ...
                    if is_mp3_like:
                        ...
                        try:
                            ...
                            if self.debug_mode:
                                self.console.print(
                                    f"[dim]Debug: Chunk {i} decoded as MP3: {len(pcm_data)} bytes PCM[/dim]"
                                )
                        except ImportError:
                            if self.debug_mode:
                                self.console.print(
                                    "[red]MP3 decoding not available - install pydub and ffmpeg[/red]"
                                )
                            ...
                            if len(audio_bytes) > 0 and len(audio_bytes) % 2 == 0:
                                ...
                                if self.debug_mode:
                                    self.console.print(
                                        f"[yellow]Debug: Chunk {i} MP3 decode failed, treating as raw PCM: {len(audio_bytes)} bytes[/yellow]"
                                    )
                            else:
                                if self.debug_mode:
                                    self.console.print(
                                        f"[yellow]Debug: Skipping chunk {i} - MP3 decode failed and invalid length[/yellow]"
                                    )
                        ...
                        if self.debug_mode:
                            self.console.print(
                                f"[dim]Debug: Chunk {i} processed as WAV: {len(pcm_data)} bytes PCM[/dim]"
                            )
                        ...
                        if len(audio_bytes) % 2 == 0:
                            ...
                            if self.debug_mode:
                                self.console.print(
                                    f"[dim]Debug: Chunk {i} treated as raw PCM: {len(audio_bytes)} bytes[/dim]"
                                )
                        else:
                            if self.debug_mode:
                                self.console.print(
                                    f"[yellow]Debug: Skipping chunk {i} - odd length {len(audio_bytes)} bytes, not valid PCM[/yellow]"
                                )
                    else:
                        if self.debug_mode:
                            self.console.print(
                                f"[yellow]Debug: Skipping chunk {i} - empty data[/yellow]"
                            )
                ...
                if processed_audio_chunks:
                    ...
                    if self.debug_mode:
                        self.console.print(
                            f"[dim]Debug: Combined audio: {len(combined_audio)} bytes PCM data, playing now...[/dim]"
                        )
    ...
    async def send_voice_message(self, duration_seconds: int = 5) -> None:
        ...
        try:
            # Record voice input
            self.console.print(
                f"[bold blue]Recording voice for {duration_seconds} seconds...[/bold blue]"
            )
            audio_data = await record_voice_input_async(
                duration_seconds=duration_seconds, debug_mode=self.debug_mode
            )
            ...
            try:
                ...
                if len(audio_array) > 0:
                    ...
                    if rms < silence_threshold:
                        ...
                        if is_interactive:
                            self.console.print(
                                f"[yellow]No speech detected (RMS: {rms:.1f}) - please speak clearly and try again[/yellow]"
                            )
                        else:
                            self.console.print(
                                f"[yellow]No audio input available (RMS: {rms:.1f}) - running in non-interactive mode[/yellow]"
                            )
                            self.console.print(
                                "[yellow]Voice recording requires an interactive terminal session with microphone access[/yellow]"
                            )
                    ...
                    self.console.print(
                        "[yellow]No audio data captured - please try recording again[/yellow]"
                    )
                    return  # Exit early without making API calls
            except Exception as e:
                if self.debug_mode:
            ...
            try:
                ...
                api_transcription = await self.client.send_audio_transcription(
                ...
                if api_transcription:
                    ...
                    self.console.print(
                        "[yellow]UG API transcription returned empty result[/yellow]"
                    )
            ...
            if not transcribed_text:
                self.console.print(
                    "[yellow]UG transcription failed - not sending to chat API[/yellow]"
                )
                return
            ...
            if self.debug_mode:
                self.console.print(
                    f"[dim]DEBUG: Sending text interaction: '{transcribed_text}'[/dim]"
                )
            ...
            try:
                ...
                async def wait_for_text_interaction():
                    response_count = 0
                    async for response in self.client.send_text_interaction(
                        transcribed_text, audio_output=self.audio_output
                    ):
                        response_count += 1
                        if self.debug_mode:
                            self.console.print(
                                f"[dim]DEBUG: Text interaction response {response_count}: {response}[/dim]"
                            )
                        ...
                        if response.get("kind") == "close":
                            if self.debug_mode:
                                self.console.print(
                                    "[dim]DEBUG: Received close message, ending text interaction[/dim]"
                                )
                            break
                ...
                if self.debug_mode:
                    self.console.print(
                        f"[dim]DEBUG: Received {len(text_responses)} text responses before timeout[/dim]"
                    )
            ...
            if audio_responses and self.audio_output:
                if self.debug_mode:
                    ...
                    self.console.print(
                        f"[dim]Debug: Collected {len(audio_responses)} voice response audio chunks ({total_bytes} bytes total), processing and merging before playback[/dim]"
                    )
                # Process audio responses - handle base64 strings and decode MP3/WAV to PCM
                processed_audio_chunks = []
                for i, audio_data in enumerate(audio_responses, 1):
                ...
                if processed_audio_chunks:
                    ...
                    if self.debug_mode:
                        self.console.print(
                            f"[dim]Debug: Combined voice audio: {len(combined_audio)} bytes PCM data, playing now...[/dim]"
                        )
                ...
                if user_input.startswith("/"):
                    ...
                    if command == "quit" or command == "exit":
                        ...
                        session.console.print(
                            "  /new <id>       - Create a new player with external ID"
                        )
                        session.console.print(
                            "  /voice [sec]    - Record and send voice message (default 3 seconds)"
                        )
                        session.console.print(
                            "  /test-audio     - Play test audio tone to verify playback"
                        )
                        session.console.print(
                            "  /toggle-voice   - Switch between voice and text conversation modes"
                        )
                        session.console.print("  /status         - Show connection status")
                        session.console.print(
                            "  /system <prompt> - Update the system prompt for the AI"
                        )
                        ...
                        session.console.print(
                            f"[dim]Current mode: [{mode_color}]{mode_indicator}[/{mode_color}][/dim]"
                        )
                        session.console.print(
                            "[dim]Use ↑/↓ arrows for persistent message history[/dim]"
                        )
                        session.console.print(
                            "[dim]Type commands and press Tab for auto-completion[/dim]"
                        )
                        continue
                    elif command == "status":
                        ...
                        if session.is_connected:
                            session.console.print(
                                f"[blue]Message history: {len(session.message_history)} messages[/blue]"
                            )
                        continue
                    elif command == "voice":
                        ...
                        if len(command_parts) > 1:
                            try:
                                ...
                                if duration < 1 or duration > 30:
                                    session.console.print(
                                        "[red]Duration must be between 1-30 seconds[/red]"
                                    )
                                    continue
                            except ValueError:
                                session.console.print(
                                    "[red]Invalid duration. Use /voice or /voice <seconds>[/red]"
                                )
                                continue
                        ...
                        try:
                            ...
                            session.console.print(
                                "[dim]Playing test audio (440Hz tone for 1 second)...[/dim]"
                            )
                        ...
                        mode_status = (
                            "VOICE CONVERSATION" if session.voice_mode else "TEXT CONVERSATION"
                        )
                        mode_color = "green" if session.voice_mode else "blue"
                        session.console.print(
                            f"[{mode_color}]Mode switched to: {mode_status}[/{mode_color}]"
                        )
                        if session.voice_mode:
                            session.console.print(
                                "[green]Press Enter to start voice recording[/green]"
                            )
                        else:
                        ...
                        if len(command_parts) < 2:
                            ...
                            session.console.print(
                                "[dim]Example: /system You are a helpful coding assistant[/dim]"
                            )
                            continue

                        new_prompt = " ".join(
                            command_parts[1:]
                        )  # Join all remaining parts as the prompt
                        try:
                        ...
                        try:
                            ...
                            session.console.print(
                                f"[green]New player federated_id: {federated_id}[/green]"
                            )
                        ...
                        session.console.print(
                            f"[{mode_color}]Debug logging {debug_status}[/{mode_color}]"
                        )
                        continue
                        ...
                        session.console.print(
                            f"[{mode_color}]Audio output {audio_status}[/{mode_color}]"
                        )
                        continue
                        ...
                        try:
                            ...
                            for i in range(p.get_device_count()):
                                ...
                                if inputs > 0:
                                    session.console.print(
                                        f"[green]🎤 {i}: {name} (Input: {inputs}ch, {default_sample_rate}Hz)[/green]"
                                    )
                                elif outputs > 0:
                                    session.console.print(
                                        f"[blue]🔊 {i}: {name} (Output: {outputs}ch)[/blue]"
                                    )
                                else:
                            ...
                            if not is_interactive:
                                session.console.print(
                                    "[yellow]⚠️  Note: Running in non-interactive mode[/yellow]"
                                )
                                session.console.print(
                                    "[yellow]   Voice recording will show RMS 0.0 (no microphone access)[/yellow]"
                                )
                                session.console.print(
                                    "[yellow]   Run 'python run_chat.py' in an interactive terminal for real voice input[/yellow]"
                                )
                            else:
                                session.console.print(
                                    "[green]✅ Interactive mode detected - microphone should work[/green]"
                                )
...
@click.command()
@click.option(
    "--player-id", help="Player federated ID to use for chat", envvar="PLAYER_FEDERATED_ID"
)
